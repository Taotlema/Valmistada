# gtfs_loader: Parses a folder of GTFS txt files into a GTFSFeed dataclass.
...
# GTFSLoader: Reads one GTFS directory and returns a fully populated GTFSFeed.
class GTFSLoader:

    def __init__(self, gtfs_dir: str):
        self.gtfs_dir = gtfs_dir

    # _path: Resolve the full path for a filename inside gtfs_dir.
    def _path(self, filename: str) -> str:
        return os.path.join(self.gtfs_dir, filename)

    # _read: Load a GTFS txt file as a DataFrame; return empty if missing.
    ...
    # load: Parse all tables and return a populated GTFSFeed.
    def load(self, city: str = "San Francisco") -> GTFSFeed:
        ...
        # Sample stop_times to stay memory-friendly — SF has 1.3 M rows
        ...
        log.info(f"Loaded {city}: {len(feed.routes)} routes, "
