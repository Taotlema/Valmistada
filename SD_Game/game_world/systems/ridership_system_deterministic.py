# ridership_system_deterministic.py
# Deterministic generation model.
# Produces synthetic daily ridership using the same rule-based network mechanics
# as Rule-Based V1 but with all stochastic noise terms removed. Every run with
# the same inputs produces identical output — useful for controlled comparisons.
...
class RidershipSystem:
    ...
    def _route_spacing_effect(self, route: RouteEntity) -> float:
        ...
        if avg_dist <= 0:    return 0.90
        if avg_dist < 0.002: return 0.93
        if avg_dist > 0.030: return 0.88
        return 1.00
    ...
    def _route_connectivity_effect(self, route: RouteEntity,
        ...
        unique_count  = len({s.entity_id for s in route.stations})
        share = unique_count / network_count
    ...
    def _historical_anchor(self, route: RouteEntity, day_type: str) -> float:
        try:
            ...
            baseline = 0.0
        return baseline if baseline >= 10 else 0.0
    ...
    def _compose_synthetic(self, route: RouteEntity, routes: List[RouteEntity],
        ...
        # No noise term — fully deterministic
        synthetic = (
            system_scale
            * share
            * mode_factor
